...
{
  // We assume the manifest file is all text, so this should be safe
  std::string contents(reinterpret_cast<const char*>(manifestFileData.ToByteArray()));

  auto pos = contents.find("Main-Class:");
  if (pos == std::string::npos)
  {
    return "";
  }

  auto startPos = pos + strlen("Main-Class:");
  auto endPos = contents.find("\r\n", startPos);
  if (endPos == std::string::npos)
  {
    endPos = contents.find("\n", startPos);
    if (endPos == std::string::npos)
    {
      endPos = contents.find("\r", startPos);
    }
  }
  ...
  if (endPos != std::string::npos)
  {
    result = contents.substr(startPos, endPos - startPos);
  }
  else
  {
    result = contents.substr(startPos);
  }
...
int VirtualMachine::GetMainClassFromJarFile(const JavaString& fileName, JavaString &mainClassName, DataBuffer& mainClassOuput)
{
  ...
  std::string nameWithForwardSlashes = mainClassNameString;
  std::replace(nameWithForwardSlashes.begin(), nameWithForwardSlashes.end(), '.', '/');
  mainClassName = JavaString::FromCString(nameWithForwardSlashes.c_str());
  nameWithForwardSlashes = nameWithForwardSlashes.append(".class");

  zipFileResult = ZipFile::GetFile(fileName.ToUtf8String().c_str(), nameWithForwardSlashes.c_str(), mainClassOuput);
  if (zipFileResult == ZIP_FILE_NOT_FOUND)
  {
    std::string nameWithBackSlashes = mainClassNameString;
    std::replace(nameWithBackSlashes.begin(), nameWithBackSlashes.end(), '.', '\\');
    nameWithBackSlashes = nameWithBackSlashes.append(".class");

    zipFileResult = ZipFile::GetFile(fileName.ToUtf8String().c_str(), nameWithBackSlashes.c_str(), mainClassOuput);
    if (zipFileResult != ZIP_OK)
    {
      m_pLogger->LogError("Could not read main class from jar file %s : %s", fileName.ToUtf8String().c_str(), ZipFile::GetError(zipFileResult));
      return zipFileResult;
    }
  }
...
void VirtualMachine::Run(const JavaString& fileName, const std::shared_ptr<IVirtualMachineState>& pInitialState, bool userCode)
{
  try
  {
    ...
    size_t slashPos = fileName.FindLast(c_PathSeparator);
    if (JavaString::NotFound() != slashPos)
    {
      path = fileName.SubString(0, slashPos + 1);
    }
    ...
    pInitialState->Execute(*pClass->GetName().get(), c_ProgramStartMethodName, c_ProgramStartMethodType);
  }
...
JavaString VirtualMachine::GetClassNameFromFileName(const JavaString& fileName)
{
...
jobject JNICALL VirtualMachine::java_lang_VMObject_clone(JNIEnv* pEnv, jobject obj, jobject objectToClone)
{
  ...
  std::shared_ptr<JavaClass> pClass = pVirtualMachineState->InitialiseClass(*pObjectToClone->GetContainedObject()->GetClass()->GetName());
...
void VirtualMachine::CreateInitialThreadObject(const std::shared_ptr<IVirtualMachineState>& pInitialState)
{
  ...
  std::shared_ptr<JavaClass> pVMThreadClass = pInitialState->InitialiseClass(c_VMThreadClassName);
  ...
  std::shared_ptr<JavaClass> pThreadClass = pInitialState->InitialiseClass(JavaString::FromCString(JVMX_T("java/lang/Thread")));
  ...
  std::shared_ptr<JavaClass> pThreadGroupClass = pInitialState->InitialiseClass(JavaString::FromCString(JVMX_T("java/lang/ThreadGroup")));
...
jint JNICALL VirtualMachine::java_lang_reflect_Constructor_getModifiers(JNIEnv* pEnv, jobject obj)
{
  ...
  std::shared_ptr<JavaClass> pActualClass = pVirtualMachineState->InitialiseClass(*pClassName);
...
jobject JNICALL VirtualMachine::java_lang_reflect_VMConstructor_construct(JNIEnv* pEnv, jobject obj, jarray args)
{
  ...
  std::shared_ptr<JavaClass> pClass = pVirtualMachineState->InitialiseClass(*pClazzName);
...
boost::intrusive_ptr<ObjectReference> VirtualMachine::BuildParameterArray(TypeParser::ParsedMethodType& methodParams, JNIEnv* pEnv)
{
  ...
  for (auto it = methodParams.parameters.begin(); it != methodParams.parameters.end(); ++it)
  {
    ...
    else if (c_JavaTypeSpecifierReference == type)
    {
      ...
      auto pClass = pVirtualMachineState->InitialiseClass(className);
...
{
  std::shared_ptr<JavaClass> pThredLocalClass = pInitialState->InitialiseClass(JavaString::FromCString(JVMX_T("java/lang/InheritableThreadLocal")));
...
jobject JNICALL VirtualMachine::java_lang_VMThrowable_fillInStackTrace(JNIEnv* pEnv, jobject obj, jobject throwAble)
{
  ...
  std::shared_ptr<JavaClass> pClassOfVMThrowable = pVirtualMachineState->InitialiseClass(JavaString::FromCString("java/lang/VMThrowable"));
...
jobject JNICALL VirtualMachine::java_lang_reflect_VMField_get(JNIEnv* pEnv, jobject obj, jobject objectToGetValueFrom)
{
  ...
  JavaString nameAsString = HelperTypes::ExtractValueFromStringObject(pFieldName);
  ...
  case e_JavaVariableTypes::Char:
  {
    ...
    auto pResultClass = pVirtualMachineState->InitialiseClass(c_ResultClassName);
